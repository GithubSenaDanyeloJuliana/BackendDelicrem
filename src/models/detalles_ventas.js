    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const insumosNecesarios = await new Promise((resolve, reject) => {
                conexion.query(
                    'SELECT id_insumo, cantidad_insumo FROM ficha_tecnica_insumos WHERE id_producto = ? AND cantidad_insumo > 0',
                    [idProducto],
                    (err, results) => {
                        if (err) return reject(err);
                        resolve(results);
            ...
            // Log para verificar los insumos necesarios
            ...
            for (const insumo of insumosNecesarios) {
                ...
                // Log para verificar las cantidades a descontar
                console.log(`Descontando ${cantidadNecesaria} de insumo con ID: ${idInsumo}`);
            ...
            console.log("Detalle de venta registrado y insumos descontados correctamente.");
            resolve(resultado);
        } catch (err) {
